...
const Home: React.FC<HomeProps> = ({ serverSideApiKeyIsSet, defaultModelId }) => {
  ...
  return (
    <>
      <Head>
        <title>Splorite</title>
        <meta name='description' content='ChatGPT but better.' />
        <meta name='viewport' content='height=device-height ,width=device-width, initial-scale=1, user-scalable=no' />
        <link rel='icon' href='/logo.svg' />
      </Head>
      {selectedConversation && (
        <main className={`flex h-screen w-screen flex-col bg-[#111827] text-sm text-white dark:text-white ${lightMode}`}>
          ...
          <div className='flex h-full w-full pt-[48px] sm:pt-0'>
            {showSidebar ? (
              <div>
                ...
                <div onClick={handleToggleChatbar} className='absolute top-0 left-0 z-10 h-full w-full bg-black opacity-70 sm:hidden'></div>
              </div>
            ) : (
              <button
            ...
            {showPromptbar ? (
              <div>
                ...
                <div onClick={handleTogglePromptbar} className='absolute top-0 left-0 z-10 h-full w-full bg-black opacity-70 sm:hidden'></div>
              </div>
            ) : (
              <button
...
export const getServerSideProps: GetServerSideProps = async ({ locale }) => {
  const defaultModelId = (process.env.DEFAULT_MODEL && Object.values(OpenAIModelID).includes(process.env.DEFAULT_MODEL as OpenAIModelID) && process.env.DEFAULT_MODEL) || fallbackModelID;
