...
export const VariableModal: FC<Props> = ({ prompt, variables, onSubmit, onClose }) => {
  const [updatedVariables, setUpdatedVariables] = useState<{ key: string; value: string }[]>(
    variables.map((variable) => ({ key: variable, value: '' })).filter((item, index, array) => array.findIndex((t) => t.key === item.key) === index)
  );
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50' onKeyDown={handleKeyDown}>
      <div
        ref={modalRef}
        className='dark:border-netural-400 inline-block max-h-[400px] transform overflow-hidden overflow-y-auto rounded-lg border border-gray-300 bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all dark:bg-[#111827] sm:my-8 sm:max-h-[600px] sm:w-full sm:max-w-lg sm:p-6 sm:align-middle'
        role='dialog'
      >
