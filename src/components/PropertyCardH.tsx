...
const PropertyCardH: FC<PropertyCardHProps> = ({ className = '', data = DEMO_DATA }) => {
  const { galleryImgs, title, href, like, saleOff, isAds, price, reviewStart, reviewCount, id } = data;
      ...
      <div className='flex flex-grow flex-col items-start p-3 sm:pr-6'>
        <div className='w-full space-y-4'>
          ...
          <div className='flex w-full items-end justify-between'>
            ...
            <span className='flex items-center justify-center rounded-lg border-2 border-secondary-500 px-2.5 py-1.5 text-sm font-medium leading-none text-secondary-500'>{`${price},000`}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className={`nc-PropertyCardH group relative overflow-hidden rounded-3xl border border-neutral-200/80 bg-white dark:border-neutral-700 dark:bg-neutral-900 ${className}`}>
