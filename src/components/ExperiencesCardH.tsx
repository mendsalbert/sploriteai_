...
const ExperiencesCardH: FC<ExperiencesCardHProps> = ({ className = '', data = DEMO_DATA as any }) => {
  const { galleryImgs, address, title, href, like, saleOff, isAds, price, reviewStart, reviewCount, author, id } = data;

  const renderSliderGallery = () => {
    return (
      <div className='relative w-full flex-shrink-0 overflow-hidden md:w-72'>
        <GallerySlider ratioClass='aspect-w-12 aspect-h-9 md:aspect-h-11' uniqueID={`ExperiencesCardH_${id}`} href={href} />
  ...
  return (
    <div className={`nc-ExperiencesCardH group relative overflow-hidden rounded-2xl border border-neutral-200/80 bg-white dark:border-neutral-700 dark:bg-neutral-900 ${className}`}>
