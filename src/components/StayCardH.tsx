...
const StayCardH: FC<StayCardHProps> = ({ className = '', data = DEMO_DATA }) => {
  const { galleryImgs, listingCategory, address, title, href, like, saleOff, isAds, price, reviewStart, reviewCount, id } = data;

  const renderSliderGallery = () => {
    return (
      <div className='relative w-full flex-shrink-0 md:w-72 '>
        <GallerySlider ratioClass='aspect-w-6 aspect-h-5' uniqueID={`StayCardH_${id}`} href={href} />
