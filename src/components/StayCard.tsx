...
const StayCard: FC<StayCardProps> = ({ size = 'default', className = '', data = DEMO_DATA }) => {
  const { galleryImgs, listingCategory, address, title, bedrooms, href, like, saleOff, isAds, price, reviewStart, reviewCount, id } = data;

  const renderSliderGallery = () => {
    return (
      <div className='relative w-full'>
        <GallerySlider uniqueID={`StayCard_${id}`} ratioClass='aspect-w-4 aspect-h-3 ' href={href} galleryClass={size === 'default' ? undefined : ''} />
      ...
      <div className={size === 'default' ? 'space-y-4 p-4' : 'space-y-1 p-3'}>
        ...
        <div className='flex items-center justify-between'>
          <span className='text-base font-semibold'>
            {price}
            {` `}
            {size === 'default' && <span className='text-sm font-normal text-neutral-500 dark:text-neutral-400'>/night</span>}
          </span>
